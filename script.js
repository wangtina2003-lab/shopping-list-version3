...
const filterKeyword = document.getElementById('filter-keyword'); // 新增：關鍵字搜尋 DOM
...
todoForm.addEventListener('submit', (e) => {
    ...
    // 自動把所有篩選器重設為「全部/空」
    filterKeyword.value = ""; // 新增：加入商品時清空搜尋關鍵字
    filterLocation.value = "all";
    filterStatus.value = "all";
    filterBuyer.value = "all";

    // 封裝一個安全的重置函式，確保在儲存完成後才執行
    const safeResetForm = () => {
        nameInput.value = "";   
        buyerInput.value = "";  
        locationInput.value = ""; 
...
function renderList() {
    ...
    const targetKeyword = filterKeyword.value.trim().toLowerCase(); // 新增：拿到當前輸入的關鍵字（並轉小寫）
    const targetLocation = filterLocation.value;
    const targetBuyer = filterBuyer.value;
    const targetStatus = filterStatus.value; 

    // 進行複合過濾 (關鍵字 + 地點 + 委託人 + 狀態 同時滿足)
    const filteredList = shoppingList.filter(item => {
        const matchKeyword = item.name.toLowerCase().includes(targetKeyword); // 新增：檢查品名是否包含關鍵字
        ...
        return matchKeyword && matchLocation && matchBuyer && matchStatus; // 必須全部滿足
    });
...
// 監聽所有篩選變更事件
filterKeyword.addEventListener('input', renderList); // 新增：當打字時，不用按搜尋鍵就會即時過濾
